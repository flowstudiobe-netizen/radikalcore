...
const defaultColors = ["#EAB308", "#F97316", "#e7ff00"];
...
export function LiquidEther({
  ...
  useEffect(() => {
    ...
    class MouseClass {
      ...
      container: HTMLElement | null = null; listenerTarget: Window | null = null;
      ...
      dispose() {
        ...
        this.listenerTarget = null; this.container = null;
      }
      ...
      onDocumentTouchStart(e: TouchEvent) {
        ...
        this.isHoverInside = true; if (this.onInteract) this.onInteract();
    ...
    class ShaderPass {
      ...
      material: THREE.RawShaderMaterial | null = null; geometry: THREE.BufferGeometry | null = null;
    ...
    class Advection extends ShaderPass {
      ...
      init() {
        ...
        geo.setAttribute("position", new THREE.BufferAttribute(new Float32Array([-1,-1,0,-1,1,0,-1,1,0,1,1,0,1,1,0,1,-1,0,1,-1,0,-1,-1,0]), 3));
        const mat = new THREE.RawShaderMaterial({ vertexShader: line_vert, fragmentShader: advection_frag, uniforms: this.uniforms! });
        this.line = new THREE.LineSegments(geo, mat); this.scene!.add(this.line);
      }
      update(args: any = {}) {
    ...
    class ExternalForce extends ShaderPass {
      mouse!: THREE.Mesh;
      constructor(simProps: any) {
        super({ output: simProps.dst }); this.init(simProps);
      }
      init(simProps: any) {
        super.init();
        const geo = new THREE.PlaneGeometry(1, 1);
        const mat = new THREE.RawShaderMaterial({ vertexShader: mouse_vert, fragmentShader: externalForce_frag, blending: THREE.AdditiveBlending, depthWrite: false, uniforms: { px: { value: simProps.cellScale }, force: { value: new THREE.Vector2(0, 0) }, center: { value: new THREE.Vector2(0, 0) }, scale: { value: new THREE.Vector2(simProps.cursor_size, simProps.cursor_size) } } });
        this.mouse = new THREE.Mesh(geo, mat); this.scene!.add(this.mouse);
      }
      update(props: any = {}) {
        const forceX = (Mouse.diff.x / 2) * (props.mouse_force || 0);
        const forceY = (Mouse.diff.y / 2) * (props.mouse_force || 0);
        const cs = props.cellScale || { x: 1, y: 1 };
        const size = props.cursor_size || 0;
        const cx = Math.min(Math.max(Mouse.coords.x, -1 + size * cs.x + cs.x * 2), 1 - size * cs.x - cs.x * 2);
        const cy = Math.min(Math.max(Mouse.coords.y, -1 + size * cs.y + cs.y * 2), 1 - size * cs.y - cs.y * 2);
        const u = (this.mouse.material as THREE.RawShaderMaterial).uniforms;
        u.force.value.set(forceX, forceY); u.center.value.set(cx, cy); u.scale.value.set(size, size);
        super.update();
      }
    }
    ...
    class Divergence extends ShaderPass {
      ...
      update(args: any = {}) { if (this.uniforms && args.vel) { this.uniforms.velocity.value = args.vel.texture; } super.update(); }
    }
    ...
    class Output {
      ...
      scene: THREE.Scene; camera: THREE.Camera; output: THREE.Mesh;
      constructor() {
    ...
    const io = new IntersectionObserver(entries => {
      ...
      if (visible) webglRef.current.start(); else webglRef.current.pause();
